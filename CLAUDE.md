...
- Automated checks per website:
  ...
  - **CMS/Tech stack detection** — WordPress (version + plugins + theme), Wix, Jimdo, Squarespace, Shopify, Joomla, Webflow, TYPO3, Drupal. Also detects React/Next.js/Vue/Angular, analytics (GA, Matomo, Hotjar, Plausible), e-commerce (WooCommerce, Shopify). Detects free CMS plans.
...
- Results stored per-lead: `websiteQuality`, `websiteScore`, `websiteIssues[]`, `websiteLoadTime`, `websiteAnalyzedAt`, `websiteTechStack`, `websiteSecurityGrade`, `websiteOpportunityScore`, `websiteComplexity`
...
- **Fahrschule / Fahrlehrer → CrashCode co-branding pitch**: these two default categories ship a dedicated campaign (`CRASHCODE_FAHRSCHULE_TEMPLATES` in `defaultCategories.js`) that offers driving schools the free, co-brandable CrashCode accident-help app for their students — not a website redesign. It uses only identity/CTA placeholders (`[Business Name]`, `[Dein Name]`, `[CALENDLY-LINK]`), no website/preview placeholders. The greeting is the generic `Sali zäme!` (no name, no comma) followed by a blank line, and the first sentence starts with a capital letter.
- **Multi-language variants (de/fr/it):** the CrashCode campaign ships in German, French, and Italian (`CRASHCODE_FAHRSCHULE_TEMPLATES_{DE,FR,IT}` → `CRASHCODE_FAHRSCHULE_TEMPLATES_BY_LANG`, registered under the campaign id `crashcode-fahrschule` in `CAMPAIGN_TEMPLATES`). A campaign category is tagged with `campaign: 'crashcode-fahrschule'`. `resolveTemplatesForLead(settings, category, lang)` applies the matching language variant on top of the base template when the category runs a multi-language campaign; German is the default/base and all variants keep the same placeholders. The language a recipient speaks is auto-detected from the lead's city/address (`guessLeadLanguage()` → French/Italian region keywords in `LANGUAGE_LOCATION_HINTS`, else German). Every server send path (`/api/email/send`, `/api/email/preview`, batch send) resolves the variant, so single-send and batch-send also pick the right language automatically.
- **Backfill:** `ensureDefaults()` in `routes/categories.js` additively applies a shipped campaign template to an already-seeded same-named category only when that category has no `templates` of its own — existing user-defined templates are never overwritten. It also backfills the `campaign` id onto matching categories seeded before the field existed, so language resolution works without a data reset.
- **Verify before sending (with language override):** the "Email Selected" bulk action opens a verification modal (`#modalBulkVerify`) before any email goes out. It groups the selected leads by the template they will actually receive (a category-specific campaign such as Fahrschule vs the global template) **and, for a multi-language campaign, by the auto-detected recipient language**, shows a rendered subject + body preview per group (via `/api/email/preview?lang=`), lists the recipients, and lets you override the language per group with a Deutsch/Français/Italiano selector (re-renders the preview and updates the per-lead send language). Only sends on "Send now". Every send path — single send, "Email Selected", and batch send — resolves the per-lead template with `getTemplatesForLead()` / `resolveTemplatesForLead()`, so a Fahrschule lead always receives the Fahrschule campaign in the correct language, never the global template. The client mirrors the language heuristic in `public/js/app.js` (`LANGUAGE_LOCATION_HINTS` / `guessLeadLanguage` / `CAMPAIGN_LANGUAGES`) — keep it in sync with `server/lib/emailService.js`.
