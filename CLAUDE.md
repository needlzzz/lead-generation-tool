...
- **Verify before sending:** the "Email Selected" bulk action opens a verification modal (`#modalBulkVerify`) before any email goes out. It groups the selected leads by the template they will actually receive (a category-specific campaign such as Fahrschule vs the global template), shows a rendered subject + body preview per group (via `/api/email/preview`), lists the recipients, and only sends on "Send now". Every send path — single send, "Email Selected", and batch send — resolves the per-lead template with `getTemplatesForLead()` / `resolveTemplatesForLead()`, so a Fahrschule lead always receives the Fahrschule campaign, never the global template.
