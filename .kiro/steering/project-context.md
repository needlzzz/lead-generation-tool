...
- **Multi-language campaigns:** a campaign category tagged with `campaign` (e.g. `crashcode-fahrschule`) ships de/fr/it variants (`CAMPAIGN_TEMPLATES` in `defaultCategories.js`). `resolveTemplatesForLead(settings, category, lang)` applies the variant for the language the recipient speaks — auto-detected from the lead's city/address (`guessLeadLanguage()`), or explicitly chosen. The "Email Selected" verification modal groups leads by campaign + detected language and offers a Deutsch/Français/Italiano override per group; single-send and batch-send auto-detect. German is the default/base. The client mirrors the detection heuristic in `public/js/app.js` — keep it in sync with `server/lib/emailService.js`.
